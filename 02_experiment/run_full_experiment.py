...
def process_job(job):
    ...
    try:
        ...
        log.info(f"  Latency: {latency:.2f}s (model={job['model']})")
...
if CHECKPOINT_FILE.exists():
    ...
    log.info(f"Checkpoint found – {len(done_keys)} jobs have already been completed, will continue.")
else:
    done_keys = set()
    results   = []
...
log.info(f"Pending jobs: {len(pending_jobs)} | In total: {len(jobs_df) + len(done_keys)}")
...
log.info(f"\nSaved: llm_lead_intent_results.csv")
log.info(f"Run finished. Log saved in: {log_filename}")